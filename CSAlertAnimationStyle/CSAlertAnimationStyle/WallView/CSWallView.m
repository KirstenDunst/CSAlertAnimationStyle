...
- (void)setModel:(CSWallItemModel *)model
      withLayout:(CSWallViewLayout *)layout
      appearance:(CSWallViewAppearance *)appearance {
...
@property (nonatomic, strong) CSWallViewLayout *wallLayout;
@property (nonatomic, strong) CSWallViewAppearance *wallAppearance;

@property (nonatomic, strong) NSArray<CSWallItemModel *> *models;

@property (nonatomic, weak) CSWallView *wallView;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
              reuseIdentifier:(NSString *)reuseIdentifier
                       layout:(CSWallViewLayout *)layout
                   appearance:(CSWallViewAppearance *)appearance {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < _models.count) {
        ...
        NSAssert([object isKindOfClass:[CSWallItemModel class]], @"** zhWallView ** - 传入的数据必须使用zhWallItemModel进行打包，不能是其它对象!");
...
- (void)itemClicked:(UIButton *)sender {
    CSWallView *wallView = self.wallView;
...
- (void)setModels:(NSArray<CSWallItemModel *> *)models {
    _models = models;
...
@implementation CSWallItemModel

+ (instancetype)modelWithImage:(UIImage *)image text:(NSString *)text {
    CSWallItemModel *model = [[CSWallItemModel alloc] init];
    model.image = image;
    model.text = text;
    return model;
}
...
// MARK -zhWallView - //
////////////////////////
@interface CSWallView () <UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) UITableView *tableView;

@end

@implementation CSWallView
...
- (CSWallViewLayout *)layout {
    id <CSWallViewDelegateConfig> config = (id <CSWallViewDelegateConfig> )self.delegate;
    if ([config respondsToSelector:@selector(layoutOfItemInWallView:)]) {
        return [config layoutOfItemInWallView:self];
    }
    return [[CSWallViewLayout alloc] init];
}

- (CSWallViewAppearance *)appearance {
    id <CSWallViewDelegateConfig> config = (id <CSWallViewDelegateConfig> )self.delegate;
    if ([config respondsToSelector:@selector(appearanceOfItemInWallView:)]) {
        return [config appearanceOfItemInWallView:self];
    }
    return [[CSWallViewAppearance alloc] init];
}

#pragma mark - Setter

- (void)setModels:(NSArray<NSArray<CSWallItemModel *> *> *)models {
    _models = models;
