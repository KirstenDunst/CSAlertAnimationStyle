...
#define CSAlertViewLineColor [UIColor colorWithHexString:@"bfbfbf"]
...
- (instancetype)initWithTitle:(NSString *)title handler:(void (^)(CSAlertButton * _Nonnull))handler {
    if (self = [super init]) {
        ...
        _horizontalLine.backgroundColor = CSAlertViewLineColor.CGColor;
        [self.layer addSublayer:_horizontalLine];
        
        _verticalLine = [CALayer layer];
        _verticalLine.backgroundColor = CSAlertViewLineColor.CGColor;
...
@implementation CSAlertStyleOne
...
- (void)addAction:(CSAlertButton *)action {
    ...
    CSAlertButton *lastAction = objc_getAssociatedObject(self, zhAlertViewActionKey);
...
- (void)clearActions:(NSArray *)subviews {
    for (UIView *subview in subviews) {
        if ([subview isKindOfClass:[CSAlertButton class]]) {
