...
@interface CSCurrentDateModel : NSObject
...
@implementation CSCurrentDateModel
...
@interface CSPickerView () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@property (nonatomic, strong) CSCurrentDateModel *currentDateModel; // 记录当前选中的数据
...
- (void)initialSelectedItem {
    ...
    CSCurrentDateModel *model = self.currentDateModel;
...
- (void)showTitleWithModel:(CSCurrentDateModel *)model {
...
// 当前时间数据模型
- (CSCurrentDateModel *)currentDateModel {
    if (!_currentDateModel) {
        _currentDateModel = [CSCurrentDateModel new];
    }
